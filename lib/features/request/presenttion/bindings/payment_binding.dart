...
class PaymentBinding extends Bindings {
  ...
  void dependencies() {
    Get.lazyPut<PaymentRemoteDataSource>(
        () => PaymentRemoteImp(dio: Get.find()));
    Get.lazyPut<PaymentRepository>(() => PaymentRepositoryImp(
        paymentRemoteDataSource: Get.find(), networkInfo: Get.find()));
    Get.lazyPut<PayLoanUseCase>(() => PayLoanUseCase(paymentRepository: Get.find()));
    Get.put(
      RequestController(payLoanUseCase: Get.find()),
    );
  }
}
